...
_CLIPGT2V_RESOURCE_PACKAGE = "clipgt2v.interactive_drive"
...
def resolve_world_model_manifest_path(path: str | Path) -> Path:
    ...
    resource_parts = (
        ("configs", raw_path.name) if len(raw_path.parts) == 1 else raw_path.parts
    )
    package_path = Path(
        str(resources.files(_CLIPGT2V_RESOURCE_PACKAGE).joinpath(*resource_parts))
    )
