...
class RendererSettings:
    ...
    """Whether collision visual flares are rendered."""
...
def load_renderer_settings(path: Path) -> RendererSettings:
    ...
    require_exact_keys(
        doc, {"schema_version", "raster", "bev", "visual_flare_enabled"}, "renderer"
    )
    ...
    return RendererSettings(
        raster=RasterConfig(**raster_values),
        bev=BevConfig(**bev_values),
        visual_flare_enabled=require_bool(
            doc["visual_flare_enabled"], "renderer.visual_flare_enabled"
        ),
    )
